...
3. **網頁筆記建立**：使用者會需要建立操作筆記網頁（例如麻醉操作筆記、藥物比較表、課程週次筆記等）。技術棧維持純 HTML/CSS/JS，不用框架、不用打包工具，雙擊即可在瀏覽器開啟，不需要啟動伺服器。**排版方式比照 `上群AH/` 這個姊妹站的樣式**（外部 `assets/ah.css`／`assets/nav.css`、頂部跨學期 dropdown 導覽、`pagehead` 大標題帶＋breadcrumb＋meta-bar、內頁左側目錄改成捲動高亮 TOC、章節改成編號 section、卡片改細線無陰影），細節見下方「資料夾結構」。**這條站線的檔案直接放在 repo 根目錄**（`index.html`／`search-index.js`／`assets/`／`大四上/`／`大三/`／`總複習/` 等皆在根目錄下，不再包在 `網頁筆記/` 子資料夾內），這樣 GitHub Pages 網址才不會多一層資料夾名稱。
   - **課程週次筆記**（會隨學期持續新增，不是一次性文件）與**生涯總複習主題頁**：走根目錄下的多檔案架構，見下方「資料夾結構」。
   ...
   - **一次性主題頁**（不屬於特定課程/週次，例如單一麻醉操作手冊、藥物比較表）：直接放在根目錄，樣式規則與「內頁」相同（見下方兩種頁面樣式說明）。

4. **上群AH**（`上群AH/`）：與「網頁筆記」這條站線平行、獨立的第三條站線，把醫院實際累積的臨床知識與技術整理成可查詢的知識庫，依**臨床科別**（內科／外科／影像診斷／麻醉／急重症／臨床病理與血檢判讀／藥物治療與劑量／技術操作與SOP）分類，不是依學期週次。技術棧、頁面樣式規則與新增流程與「網頁筆記」完全一致（兩站共用同一套排版語言，只是頂部導覽的分類軸不同：AH 是臨床科別、網頁筆記是學期），詳見 `上群AH/CLAUDE.md`。
...
- **「網頁筆記」這條站線的檔案直接放在 repo 根目錄**（不再有獨立的 `網頁筆記/` 子資料夾——這層資料夾在 2026-09-17 已移除，讓 GitHub Pages 網址是乾淨的 `https://yc-vetalk.github.io/DVM-note/` 而非 `.../DVM-note/網頁筆記/`），結構如下：
  ```
  （repo 根目錄）
