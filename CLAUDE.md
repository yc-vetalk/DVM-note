...
3. **網頁筆記建立**：使用者會需要建立操作筆記網頁（例如麻醉操作筆記、藥物比較表、課程週次筆記等）。技術棧維持純 HTML/CSS/JS，不用框架、不用打包工具，雙擊即可在瀏覽器開啟，不需要啟動伺服器。**排版方式比照 `上群AH/` 這個姊妹站的樣式**（外部 `assets/ah.css`／`assets/nav.css`、頂部跨學期 dropdown 導覽、`pagehead` 大標題帶＋breadcrumb＋meta-bar、內頁左側目錄改成捲動高亮 TOC、章節改成編號 section、卡片改細線無陰影），細節見下方「資料夾結構」。**這條站線的檔案都收在 `DVM-note/` 這個獨立資料夾內**（`index.html`／`search-index.js`／`assets/`／`大四上/`／`大三/`／`總複習/` 皆在 `DVM-note/` 下，不與 `上群AH/`、病歷資料等其他個人檔案混放在 repo 根目錄），**下方「資料夾結構」列出的路徑（`assets/`、`大四上/` 等）除非特別註明，否則都是相對於 `DVM-note/` 這一層**。repo 根目錄另有一個極簡的 `index.html` 跳轉頁（`<meta http-equiv="refresh">` 導向 `DVM-note/index.html`），只是為了讓 GitHub Pages 網址可以直接打 `https://yc-vetalk.github.io/DVM-note/` 不必手動多打一層路徑，內容本身仍在 `DVM-note/` 資料夾內，不要把筆記頁面誤放到 repo 根目錄。
   - **課程週次筆記**（會隨學期持續新增，不是一次性文件）與**生涯總複習主題頁**：走 `DVM-note/` 下的多檔案架構，見下方「資料夾結構」。
   - **站台品牌文字**：header-brand（每一頁左上角）固定顯示 `<h1>yc._vetalk</h1>` ＋ `<div class="subtitle">DVM Course Notes · 大一～大四</div>`；頂層首頁的大標題（`.pagehead__title`）是 `DVM_note`。新增頁面時複製既有頁面模板即可帶到這兩處，不要改回舊名「上群 網頁筆記」（`<title>` 標籤同樣一律用 `yc._vetalk`，不要用資料夾/repo 名稱）。
   - **一次性主題頁**（不屬於特定課程/週次，例如單一麻醉操作手冊、藥物比較表）：直接放在 `DVM-note/` 根目錄，樣式規則與「內頁」相同（見下方兩種頁面樣式說明）。

4. **上群AH**（`上群AH/`）：與「網頁筆記」這條站線平行、獨立的第三條站線（放在 repo 根目錄，與 `DVM-note/` 同一層，彼此不互相巢狀），把醫院實際累積的臨床知識與技術整理成可查詢的知識庫，依**臨床科別**（內科／外科／影像診斷／麻醉／急重症／臨床病理與血檢判讀／藥物治療與劑量／技術操作與SOP）分類，不是依學期週次。技術棧、頁面樣式規則與新增流程與「網頁筆記」完全一致（兩站共用同一套排版語言，只是頂部導覽的分類軸不同：AH 是臨床科別、網頁筆記是學期），詳見 `上群AH/CLAUDE.md`。
...
- `DVM-note/`：「網頁筆記」這條站線的完整內容，與 `上群AH/`、病歷資料等其他個人資料夾完全分開、自成一個獨立資料夾（2026-09-17 整理，避免站台檔案跟個人病例資料混在 repo 根目錄）。repo 根目錄另有一個極簡 `index.html` 跳轉頁導向 `DVM-note/index.html`，純粹讓 GitHub Pages 網址維持 `https://yc-vetalk.github.io/DVM-note/` 不必多打路徑，不是內容本體。`DVM-note/` 內部結構如下（以下路徑皆相對於 `DVM-note/`）：
  ```
  DVM-note/
...
- 與「大三／大四當學期週次筆記」平行、獨立的第二條站線，用來放**跨學年、依主題（非週次）整理的長期複習內容**，資料來源是 `獸醫總複習筆記/` 底下依科目分類的舊講義（可能橫跨多個學年度，例如 112-1、113-1，原則上以較新學年版本為主、舊版只補充差異內容）。
  ...
  - **圖片**：解剖／組織型態類主題務必配圖，不能只有文字。做法：
    1. 用 `_scripts/extract_pdf_images.py`（讀取 `.venv` 的 `pymupdf`／`Pillow`）從來源 PDF 擷取指定頁碼存成 PNG，加 `--autocrop` 去除頁面空白邊界；範例：`./.venv/bin/python _scripts/extract_pdf_images.py "<pdf路徑>" 總複習/<科目>/img/<主題> --pages 6,12 --prefix skull --dpi 220 --autocrop`
